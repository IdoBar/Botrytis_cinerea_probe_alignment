---
title: "Whole Genome Sequencing of <i>Botrytis cinerea</i> Isolates"
author: "Ido Bar"
date: "18 July 2017"
...
isolate_table <- readxl::read_excel("./sample_info/Botrytis_isolates_for_WGS_CAGRF20074.xlsx") %>% 
  ...
  mutate(Sample_name=gsub("\\s+", "_", Sample_name))
...
# Experimental Design
DNA was extracted from 20 strains of _Botrytis cinerea_ and 2 _Botrytis fabae_ isolates and sent for Whole-Genome-Sequencing (WGS) at the Australian Genome Research Facility (AGRF, Melbourne) on a single flowcell an Illumina NovaSeq, producing 150 bp short paired-end reads (run name CAGRF20074).  
...
# Aims
* Verify conservation of the molecular probes regions within _B. cinerea_ isolates
* Confirm molecular probes specificity in comparison to _B. fabae_ isolates 
...
2. Reads deduplication
3. Mapping reads to a reference genome (keep unmapped)
4. Alignment visualisation in probe regions
...
The alignment files of the samples (excluding the ones that did not pass QC) were combined into a single file that was used cropped to the primer regions to focus and visualise the alignments.
_Prepare a `fasta` file with the primer sequences and copy to the server_

```{bash crop_bams}
# Prepare Botrytis reference genome as a Blast db
GENOME="$HOME/scratch/data/reference_genomes/Fungal_genomes/B.cinerea/GCA_000143535.4_ASM14353v4"
 makeblastdb -dbtype nucl -in ${GENOME}_genomic.fna -out /export/home/s2978925/etc/tools/Annotation/protein_dbs/Bc_B10.05
# Blast primer sequences against the genome 
blastn -query primer_seqs.fasta -db /export/home/s2978925/etc/tools/Annotation/protein_dbs/Bc_B10.05 -outfmt 6 -task "blastn-short"  -evalue 1e-3 > primer_seqs.outfmt6
# get gene sequences and BLAST against the genome
# find Nep1 genomic sequence (based on its gene id)
esearch -db "gene" -query "5433262" | efetch -format fasta 
# download the genomic sequences as fasta
efetch -db nuccore -id 'NC_037311.1' -seq_start 2013643 -seq_stop 2016038 -format fasta > primer_gene_seqs.fasta

# Save the primer genomic coordinates and edit manualy (at this stage) to take ~2-5kb region around each region
gawk '{printf "%s:%s-%s\n", $2, $9,$10}' primer_seqs.outfmt6 > primer_regions.txt
# crop both bam files to these regions (:::: takes the content of the file as input)
parallel --dry-run "sambamba view -f bam {2} {1} > {2.}_{=1 s/:/_/ =}.bam" ::: $( cat primer_regions.txt) ::: *.combined.bam | sed "s/'//g" | parallel
# index the files (can also be done from R)
parallel sambamba index ::: *csorted.combined_CP*.bam
# copy bam files across
```
